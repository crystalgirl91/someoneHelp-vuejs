<template>
	<div class="my_setting_personal">
		...
		<group>
			<cell title="姓 名：" v-bind:value="data.UName" :is-link="true" v-on:click="show1=true"></cell>
			<cell title="密 码：" value="*******" :is-link="true" v-on:click="show2=true"></cell>
			<cell title="联系电话：" v-bind:value="data.Telephone" :is-link="true" v-on:click="show3=true"></cell>
			<cell title="邮 箱：" v-bind:value="data.Email" :is-link="true" v-on:click="show4=true"></cell>
		</group>
			...
			<div class="m_dialog_content">
				<div class="m-dialog-body">
					<group style="margin-bottom:.45rem">
						<x-input readonly title="姓 名：" v-bind:value="data.UName"></x-input>
		...
		<dialog :show.sync="show3" :hide-on-blur="true">
			<div class="m_dialog_content">
				<div class="m-dialog-body">
					<group style="margin-bottom:.45rem">
						<x-input readonly title="联系电话：" v-bind:value="data.Telephone"></x-input>
						<x-input title="新联系电话：" :value.sync="Telephone" placeholder="请输入..."></x-input>
					</group>
		...
		<dialog :show.sync="show4" :hide-on-blur="true">
			<div class="m_dialog_content">
				<div class="m-dialog-body">
					<group style="margin-bottom:.45rem">
						<x-input readonly title="邮 箱：" v-bind:value="data.Email"></x-input>
	...
	export default {
		...
		methods:{
							...
							wx.uploadImage({
								...
								success: function (res) {
									...
									_this.$http.post('/api/NoPermissions/UploadFile',{"FType":0,"serverId":serverId}).then(fileres => {
										...
										//设置头像
										_this.$http.post('/api/Teacher/UpdateUserHeadPortraits',_this.data).then(fileres => {
											//设置头像
			...
			submit:function(type){
				switch(type){
					case 1:
					    this.$http.post('/api/Teacher/UpdateUserUName',{"UName":this.UName}).then(res => {
							if(res.body.Data){
					...
					case 2:
						if(this.NewLoginPwd==this.OKNewLoginPwd){
							this.$http.post('/api/Teacher/UpdateUserPwd',{"YLoginPwd":this.YLoginPwd,"NewLoginPwd":this.NewLoginPwd,"OKNewLoginPwd":this.OKNewLoginPwd}).then(res => {
								if(res.body.Data){
									alert("操作成功！");
								}
							});
						...
						break;
					case 3:
					this.$http.post('/api/Teacher/UpdateUserTelephone',{"Telephone":this.Telephone}).then(res => {
							if(res.body.Data){
						...
						break;
					case 4:
					this.$http.post('/api/Teacher/UpdateUserEmail',{"Email":this.Email}).then(res => {
							if(res.body.Data){
...
<style>
	...
	.my_setting_personal .m-dialog-body .weui_cell .weui_label{
		word-spacing: .2rem;
	}

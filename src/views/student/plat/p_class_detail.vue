<template>
	...
	<div class="plat_class_detail">
		<table class="class_detail_tabel">
			...
			<tbody>
				<tr v-link="{'name':'plat_student',params:{'PK_TID':id}}">
					<td class="td"><i class="icon icon-Num-of-participants color-blue"></i><span class="label">同班同学</span>{{data.UserNumber}}人</td>
					<td class="td"><i class="icon icon-header color-blue"></i><span class="label">班长</span>{{data.ClassPresident}}</td>
				</tr>
				<tr>
					<td class="td"><i class="icon icon-score color-red"></i><span class="label">总学分</span>{{data.Credit}}</td>
					<td class="td"><i class="icon icon-score color-red"></i><span class="label">已学学分</span>{{data.LearnCredit}}</td>
				</tr>
			</tbody>
		</table>
		<div class="bg-bar-gray"></div>
...
<style>
	...
	.plat_class_detail .plat_course .title .title-border-l{
		font-weight: normal;
	}
...
<script>
	...
	export default{
		...
		methods:{
			...
			getData:function(){
				return this.$http.post("/API/Student/GetPhaseStudy",{
					"PK_CGID":this.group
				}).then(res => {

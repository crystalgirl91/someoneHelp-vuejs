<template>
	<div class="answer-sub my_class_detail_comment">
		<m-header title="评论" :left-ops="{showBack:false}" :bg-style="{'backgroundColor':'#f0f4f3'}">
		...
		<group class="group-nomargint">
			<x-textarea :rows="7" placeholder="输入文字..." :value.sync="data.MediaContent.Content"></x-textarea>
		</group>
		<tab active-color="#ff9100" :line-width="0">
			...
			<div class="padding-block">
				<i class="icon-biger icon-video" v-show="!ispause"  v-on:click="startrecording()" ></i>
		...
		<div class="block" v-if="selected === 2">
			<dl class="upload-pic-list clearfix">
				<dd class="upload-pic" v-for="m in images">
					<i v-on:click="deleteimage(m)" class="icon-abs icon-close2"></i>
					<img :src="m" alt="">
				</dd>
			...
			<div class="m_dialog_content">
				...
				<div class="m-dialog-body">
					<label class="checkbox-inline">
						<input type="checkbox" v-model="" name="type">
						<span>默认全员可查看</span>
					</label>
					<label class="checkbox-inline">
						<input type="checkbox">
					...
					<x-button class="weui_btn_normal" type="normal" text="确定"></x-button>
				</div>
			</div>
		</dialog>
	</div>
...
<script type="text/javascript">
  ...
  export default {
        ...
        methods:{
            ...
            submit:function(){
			  for(var i=0;i<this.images.length;i++){
				  this.data.MediaContent.Media[this.data.MediaContent.Media.length]={MType:0,MLength:0,MUrl:this.images[i]};
			  }
			  for(var i=0;i<this.audio.length;i++){
				  this.data.MediaContent.Media[this.data.MediaContent.Media.length]={MType:1,MLength:0,MUrl:this.audio[i].url};
			  }
			  this.data.PK_TID=this.$route.params.id;
              this.$http.post('/api/Teacher/SetTeamComment',this.data).then(res => {
                if(res.body.Data){
                  	history.go(-1);
                }
              });
...
<style>
  ...
  background-color: #f0f4f3;
	...
	.my_class_detail_comment .m-dialog-body{
		padding-top: .1rem;
	}

	.my_class_detail_comment .checkbox-inline{
		margin-bottom: .1rem;
		display: block;
		font-size: .30rem;
		text-align: left;
	...
	.my_class_detail_comment .checkbox-inline input[type='checkbox']{
